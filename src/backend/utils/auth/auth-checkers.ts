...
export async function assertCanCreate<T extends IAuthorizable>(entity: T, ctx: IRequestContext): Promise<T> {
  if (!entity.authorizationChecker) {
    return entity;
  }

  if (!await entity.authorizationChecker.canCreate(ctx)) {
    throw new UnauthorizedError();
  }
